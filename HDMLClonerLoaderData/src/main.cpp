...
int __fastcall __window_proc(int self, void *notUsed, unsigned int uMsg, unsigned int wParam, unsigned int lParam)
{
	...
	if (uMsg == WM_NCRBUTTONUP || uMsg == WM_RBUTTONUP)
	{
		...
		AppendMenu(mainMenu, 0, 0xA000A, L"Poka¿ przyciski");
		if (_show_buttons)
		{
		...
		AppendMenu(mainMenu, 0, MF_SEPARATOR, 0);
		AppendMenu(mainMenu, 0, 0xA000B, L"Przechwyæ obraz");
		AppendMenu(mainMenu, 0, MF_SEPARATOR, 0);

		AppendMenu(mainMenu, 0, 0xA000C, _isSound ? L"Wy³¹cz dŸwiêk" : L"W³¹cz dŸwiêk");
		AppendMenu(mainMenu, 0, MF_SEPARATOR, 0);

		//AppendMenu(mainMenu, 0, 0xA0008, L"Udostêpnianie obrazu");
		...
		AppendMenu(mainMenu, 0, MF_SEPARATOR, 0);
		AppendMenu(mainMenu, 0, 0xA0004, L"WyjdŸ");
